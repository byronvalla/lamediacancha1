...
(async function init(){
  ...
  if(!file){ $("#articleTitle").textContent="Noticia no encontrada"; return; }
  const data = await loadJSON(`content/news/${file}`);
  const cover = safe(data.coverImage,"https://source.unsplash.com/Fe3mpcOMQnU/1600x900");
  $("#articleCover").style.backgroundImage = `linear-gradient(180deg, rgba(0,0,0,.18), rgba(0,0,0,.42)), url('${cover}')`;
  $("#articleTitle").textContent = safe(data.title,"Noticia");
  $("#articleExcerpt").textContent = safe(data.excerpt,"");
  $("#articleMeta").textContent = [safe(data.league,""), formatDate(data.date||""), safe(data.readingTime,"")].filter(Boolean).join(" • ");
  const content = safe(data.content,"").trim();
  $("#articleContent").innerHTML = content ? content.split("\n").filter(Boolean).map(t=>`<p>${t}</p>`).join("") : "<p>Contenido en desarrollo.</p>";
})();
